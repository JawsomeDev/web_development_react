...
- 간단한 API 호출 코드 : 불필요한 상태 변수를 없애고ㅡ 가독성이 증진됨.
...
- queryFn은 데이터를 가져오는 함수이며 '프로미스로 반환'해야 한다.
...
더이상 App.jsx에 한 번에 전부 다 쓰지 않고, 필요 기능을 Component로 분할 했다는 점

그리고 Rpositories라는 Component 자체를 App.jsx에 집어넣었습니다.
...
- 그리고 이상의 앱은 리액트 쿼리 라이브러리를 이용해서 리포지토리 정보들을 가지고 오고, 리액트 쿼리 내장 기능을 이용하여 요청 상태를 쉽게(useState를 쓸 때보다) 처리할 수 있었습니다. 리액트 쿼리가 데이터 관리와 캐싱을 처리하기 때문에 응답 데이터를 저장하기 위한 상태
(data, setData)가 필요하지 않았습니다.
...
이전 예시에서는 useMutation을 통해서 서버에 새로운 자동차를 추가했습니다. 만약 모든 자동차를 가져오는 쿼리가 있고, 그 쿼리의 ID가 cars라면 새 자동차가 성공적으로 추가된 후에 해당 쿼리를 무효화해야할 것입니다.
...
일경우에 저희가 vite project 만들 때처럼 버전을 명시하는 것이 가능합니다.
...
필터링 / 정렬 / 피벗 등 유용한 기능이 있구요. 유무료버전이있는데 저희는 무료 쓸겁니다.
...
```
를 import 했습니다. 다른 css 파일들의 경우에은 node_modules 폴더(의존성을 모아둔)에 ag-grid-community를 확인하시면 됩니다.
